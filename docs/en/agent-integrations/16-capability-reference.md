...
| opencode | v1 `session.idle` / v2 execution completion: after flush, `pending_tokens ≥ 20000` triggers commit, keep 10 | `session.deleted` / v1 `session.error`: forced commit; v1 dispose / v2 cleanup: forced commit | v1 fires before compacting and after compacted; v2 fires once after `session.compaction.ended` |
...
- **Behavior notes**: `commitTokenThreshold` is 20000 and commit timeout is 30000ms. v2 checks the threshold after every succeeded, failed, or interrupted execution and force-commits on session deletion, completed compaction, and cleanup; a failed or interrupted execution retains state. v2 handles only sessions of its own location and stores its capture cursor in plugin storage. v2 cleanup runs after 60 minutes of inactivity, service shutdown, or local-plugin hot reload. v2 has no toast API and logs notices instead. Opening injection is attempted once per session ([§3.2.3](#_3-2-3-profile-opening-injection)).
